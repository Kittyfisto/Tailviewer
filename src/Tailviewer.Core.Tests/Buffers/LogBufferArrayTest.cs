...
namespace Tailviewer.Core.Tests.Buffers
{
		...
		: AbstractReadOnlyLogBufferTest
	{
		...
		{
			var buffer = new LogBufferArray(count, new List<IColumnDescriptor> {Core.Columns.RawContent, Core.Columns.DeltaTime});
		...
		public void TestCopyFromPartial1()
		{
			...
			buffer.CopyFrom(Core.Columns.Timestamp, 0, new DateTime?[] { unusedTimestamp, timestamp }, 1, 1);
			buffer[0].Timestamp.Should().Be(timestamp, "Because we've just copied this timestamp to the buffer");
		}
		...
		public void TestCopyFromPartial2()
		{
			...
			buffer.CopyFrom(Core.Columns.Timestamp, 0, new DateTime?[] { timestamp, unusedTimestamp }, 0, 1);
			buffer[0].Timestamp.Should().Be(timestamp, "Because we've just copied this timestamp to the buffer");
		}
		...
		{
			var buffer = new LogBufferArray(3, Core.Columns.LineNumber);
