...
namespace Tailviewer.Test.BusinessLogic.DataSources
{
	...
	public sealed class SingleDataSourceTest
	{
		...
		public void TestLevelCount1()
		{
			...
			{
				_scheduler.Run(2);
				dataSource.UnfilteredLogFile.Property(x => x.GetProperty(LogFileProperties.PercentageProcessed)).ShouldEventually().Be(Percentage.HundredPercent);
		...
		public void TestClearScreen()
		{
			...
			{
				_scheduler.Run(3);
				dataSource.FilteredLogFile.GetProperty(LogFileProperties.LogEntryCount).Should().Be(2);

				dataSource.ClearScreen();
				_scheduler.Run(3);
				dataSource.FilteredLogFile.GetProperty(LogFileProperties.LogEntryCount).Should().Be(0, "because we've just cleared the screen");

				logFile.AddEntry("Hello!");
				_scheduler.Run(3);
				dataSource.FilteredLogFile.GetProperty(LogFileProperties.LogEntryCount).Should().Be(1, "because newer log entries should still appear");
		...
		public void TestClearScreenShowAll()
		{
			...
			using (var dataSource = new SingleDataSource(_scheduler, settings, logFile, TimeSpan.Zero))
			{
				...
				dataSource.ShowAll();
				_scheduler.RunOnce();
				dataSource.FilteredLogFile.GetProperty(LogFileProperties.LogEntryCount).Should().Be(2, "because we've just shown everything again");
			}
		}
