...
namespace Tailviewer.Test.BusinessLogic.Filters
{
	...
	public sealed class SubstringFilterTest
	{
		...
		public void TestMatch1()
		{
			...
			new Action(() => filter.Match(new LogEntry2(LogFileColumns.Minimum){RawContent = null}, matches)).Should().NotThrow();
		...
		public void TestMatch2()
		{
			...
			var matches = new List<LogLineMatch>();
			filter.Match(new LogEntry2(LogFileColumns.Minimum){RawContent = "Foobar"}, matches);
		...
		{
			var filter = new SubstringFilter("a", true);
			filter.PassesFilter(new LogEntry2(LogFileColumns.Minimum){RawContent = null}).Should().BeFalse();
		}
