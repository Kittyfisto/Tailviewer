...
namespace Tailviewer.Test.BusinessLogic.Filters
{
	...
	public sealed class EmptyLogLineFilterTest
	{
		...
		public void TestPassesFilter1()
		{
			...
			filter.PassesFilter(new LogEntry2(LogFileColumns.Minimum)).Should().BeFalse("because the given logline is completely empty");
			filter.PassesFilter(new LogEntry2(LogFileColumns.Minimum){Index=0, RawContent ="", LogLevel = LevelFlags.All})
				.Should()
				.BeFalse("because the given line contains only an empty message");
			filter.PassesFilter(new LogEntry2(LogFileColumns.Minimum){Index=0, RawContent = " ", LogLevel = LevelFlags.All})
				.Should()
				.BeFalse("because the given line contains only spaces");
			filter.PassesFilter(new LogEntry2(LogFileColumns.Minimum){Index=0, RawContent = " \t \r\n", LogLevel = LevelFlags.All})
				.Should()
				.BeFalse("because the given line contains only whitespace");
			filter.PassesFilter(new LogEntry2(LogFileColumns.Minimum){Index=0, RawContent = " s    \t", LogLevel = LevelFlags.All})
				.Should()
