namespace Tailviewer.Core.Buffers
{
	...
	public static class LogBufferExtensions
	{
		...
		///    holds data for the given <paramref name="column"/> and the returned buffer combines both the
		///    <paramref name="that" /> as well as the temporary buffer into one view.
		/// </summary>
		/// <remarks>
		///    This method exists for those cases, where, in order to fulfill a certain <see cref="ILogSource.GetEntries(System.Collections.Generic.IReadOnlyList{LogLineIndex}, ILogBuffer, int, LogFileQueryOptions)"/>
		...
		/// <returns></returns>
		public static ILogBuffer CreateViewWithAdditionalColumn(this ILogBuffer that, IColumnDescriptor column)
		{
			if (that.Contains(column))
				return that;
