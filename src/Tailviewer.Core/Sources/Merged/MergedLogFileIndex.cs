...
namespace Tailviewer.Core.Sources.Merged
{
	...
	internal sealed class MergedLogFileIndex
	{
		...
		public MergedLogFileIndex(params ILogSource[] sources)
		{
		...
		public void GetSourceIds(IReadOnlyList<LogLineIndex> sourceIndices, LogLineSourceId[] destination, int destinationIndex)
		{
			lock (_syncRoot)
			{
				for (int i = 0; i < sourceIndices.Count; ++i)
				{
					...
					{
						destination[destinationIndex + i] = LogColumns.SourceId.DefaultValue;
					}
				}
			}
		}
		...
		///     Only retrieves the <see cref="LogColumns.Index" />, <see cref="LogColumns.LogEntryIndex" />
		///     and <see cref="LogColumns.Timestamp" /> columns (as these are the only ones required in order to
		///     merge stuff).
		/// </summary>
		/// <param name="pendingModifications"></param>
		/// <returns></returns>
		...
		private IEnumerable<MergedLogLineIndex> CreateIndices(IReadOnlyLogBuffer buffer, byte logFileIndex)
		{
			...
			{
				var index = entry.GetValue(LogColumns.Index);
				var entryIndex = entry.GetValue(LogColumns.LogEntryIndex);
				var timestamp = entry.GetValue(LogColumns.Timestamp);
