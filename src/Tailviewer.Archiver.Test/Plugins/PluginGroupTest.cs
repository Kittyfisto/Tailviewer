...
namespace Tailviewer.Archiver.Test.Plugins
{
	...
	public sealed class PluginGroupTest
	{
			...
			var index = new PluginPackageIndex
			{
				...
				ImplementedPluginInterfaces = new List<PluginInterfaceImplementation>
				{
					new PluginInterfaceImplementation
					{
						...
						InterfaceTypename = typeof(ILogEntryParserPlugin).FullName
					}
				}
			};
			PluginGroup.FindCompatibilityErrors(index).Should().BeEquivalentTo(new object[]
			{
				new PluginError($"The plugin implements an older version of '{typeof(ILogEntryParserPlugin).FullName}'. It must target the current version in order to be usable!")
			});
		}
		...
		public void TestPluginInterfaceTooNew()
		{
			...
			PluginGroup.FindCompatibilityErrors(index).Should().BeEquivalentTo(new object[]
			{
				new PluginError($"The plugin implements a newer version of '{typeof(ILogEntryParserPlugin).FullName}'. It must target the current version in order to be usable!")
			});
		}
