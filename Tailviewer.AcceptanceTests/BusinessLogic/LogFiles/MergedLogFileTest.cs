...
namespace Tailviewer.AcceptanceTests.BusinessLogic.LogFiles
{
	...
	public sealed class MergedLogFileTest
	{
		...
		{
			using (var source = new TestLogFile(_scheduler, LogFileRealTest.File20Mb))
			using (var merged = new MergedLogFile(_scheduler, TimeSpan.FromMilliseconds(1), source))
			{
				...
				merged.Size.Should().Be(source.Size);
		...
		{
			using (var source1 = new TestLogFile(_scheduler, LogFileRealTest.FileTestLive1))
			using (var source2 = new TestLogFile(_scheduler, LogFileRealTest.FileTestLive2))
